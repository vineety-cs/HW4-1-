	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			if (currentChar >= 'A' && currentChar <= 'Z') {
				...
				return "*** Error *** An invalid character has been found in your password, alphanumeric and special characters only.";
			}
			if (currentCharNdx >= 7) {
		...
		String errMessage = "";
		if (!foundUpperCase)
			errMessage += "is missing an uppercase letter, ";
		
		if (!foundLowerCase)
			errMessage += "is missing a lowercase letter, ";
		
		if (!foundNumericDigit)
			errMessage += "is missing a numeric digit, ";
			
		if (!foundSpecialChar)
			errMessage += "is missing a special character, ";
			
		if (!foundLongEnough)
			errMessage += "is not long enough (8 characters minimum), ";
		...
		passwordIndexofError = currentCharNdx;
		return "*** Error *** Your password " + errMessage;
